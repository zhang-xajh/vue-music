	...
	export default {
		...
		methods: {
			...
			_pad(num, n = 2) {
 			...
 			_getPosAndScale() {
				const targetWidth = 40//  小圆宽
				const paddingLeft = 40// 小圆一半加padding-left
				const paddingBottom = 30// mini的高度一半
				const paddingTop = 80// middle定位的top就是距顶部距离
				const width = window.innerWidth * 0.8// 大圆宽度
				const scale = targetWidth / width
				const x = -(window.innerWidth / 2 - paddingLeft)// 水平需要的偏移
